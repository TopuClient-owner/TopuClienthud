...
public final class TopuUtilityHud {
    ...
    private static boolean on(String id) {
        for (TopuUtilityManager.Utility u : TopuUtilityManager.ALL) if (u.id().equals(id)) return TopuUtilityManager.isEnabled(u);
        return false;
    }
    ...
    private static void tick(MinecraftClient client) {
        ...
        if (zoom && !lastZoom) { savedFov = client.options.getFov().getValue(); client.options.getFov().setValue(Math.min(savedFov, 35)); }
        else if (!zoom && lastZoom) client.options.getFov().setValue(savedFov);
        lastZoom = zoom;

        boolean fullbright = on("fullbright");
        if (fullbright && !lastFullbright) { savedGamma = client.options.getGamma().getValue(); client.options.getGamma().setValue(16.0D); }
        else if (!fullbright && lastFullbright) client.options.getGamma().setValue(savedGamma);
        lastFullbright = fullbright;

        boolean fpsLimit = on("fpslimit");
        if (fpsLimit && !lastFpsLimit) { savedMaxFps = client.options.getMaxFps().getValue(); client.options.getMaxFps().setValue(Math.min(savedMaxFps, 240)); }
        else if (!fpsLimit && lastFpsLimit) client.options.getMaxFps().setValue(savedMaxFps);
    ...
    private static void render(MinecraftClient client, DrawContext draw) {
        ...
        for (TopuUtilityManager.Utility u : TopuUtilityManager.ALL) {
            if (!on(u.id()) || isCore(u.id()) || u.id().equals("fpsgraph")) continue;
            String value = value(client, u.id());
            if (value == null) continue;
            int px = x + col * 109, py = y + row * 18;
    ...
    private static boolean isCore(String id) {
        return id.equals("fps") || id.equals("ping") || id.equals("tps") || id.equals("cps") || id.equals("combo") || id.equals("armor") || id.equals("effects") || id.equals("potions") || id.equals("gapples") || id.equals("totems") || id.equals("enemyhp") || id.equals("cooldown") || id.equals("warning") || id.equals("blockoverlay") || id.equals("keystrokes") || id.equals("memory") || id.equals("autosprint") || id.equals("togglesneak");
    }
    ...
    private static String value(MinecraftClient client, String id) {
        ...
        return switch (id) {
            ...
            case "zoom" -> "ACTIVE";
            case "fullbright" -> "ACTIVE";
            case "fpslimit" -> "240 FPS";
            case "lowlatency" -> "ACTIVE";
            case "cleanhud" -> "ACTIVE";
            case "minimalparticles" -> "ACTIVE";
            case "itemphysics" -> "ACTIVE";
            ...
            case "chunk" -> p.getChunkPos().x + ", " + p.getChunkPos().z;
            case "light" -> Integer.toString(client.world.getLightLevel(p.getBlockPos()));
            case "target" -> target(client);
            case "durability" -> durability(p.getMainHandStack());
            ...
            case "difficulty" -> client.world.getDifficulty().getName();
            case "session" -> formatDuration(Duration.ofMillis(System.currentTimeMillis() - SESSION_START));
            default -> null;
        };
    }
