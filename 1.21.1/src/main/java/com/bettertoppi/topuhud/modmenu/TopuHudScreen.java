...
public final class TopuHudScreen extends Screen {
    ...
    public TopuHudScreen(Screen parent) {
        super(Text.literal("Topu Utility HUD"));
        this.parent = parent;
    }
    ...
    private void clearAndBuild() {
        ...
        for (TopuUtilityManager.Category c : TopuUtilityManager.Category.values()) {
            addCategory(pretty(c.name()), c, left, y); y += 26;
        }
        rebuildButtons();
    }
    ...
    private void addCategory(String label, TopuUtilityManager.Category c, int x, int y) {
        this.addDrawableChild(ButtonWidget.builder(Text.literal(label), btn -> {
            category = c; scroll = 0; rebuildButtons();
        }).dimensions(x, y, 132, 22).build());
    }

    private void rebuildButtons() {
        List<net.minecraft.client.gui.widget.ClickableWidget> remove = new ArrayList<>();
        for (var child : this.children()) {
            if (child instanceof ButtonWidget b && b.getX() > 160) remove.add(b);
        }
        remove.forEach(this::remove);

        TopuUtilityManager.Utility[] utilities = TopuUtilityManager.search(
                searchBox == null ? "" : searchBox.getText(), category);
        ...
        for (int i = 0; i < visible; i++) {
            ...
            ButtonWidget b = ButtonWidget.builder(
                    Text.literal(u.name() + "  •  " + (TopuUtilityManager.isEnabled(u) ? "ON" : "OFF")), btn -> {
                        boolean value = !TopuUtilityManager.isEnabled(u);
                        TopuUtilityManager.setEnabled(u, value);
                        ConfigManager.save();
