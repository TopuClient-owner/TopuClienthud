...
public final class HudManager {
    ...
    public enum HudId {
        ARMOR, FPS, PING, TPS, CPS, COMBO, TOTEM, POTION, EFFECTS, GAPPLE,
        WARNING, ENEMY, COOLDOWN, BLOCK_OVERLAY, KEYSTROKES, MEMORY
    }
    ...
    public static void setEditMode(boolean value) {
        ...
        if (value) { if (c.currentScreen == null) c.mouse.unlockCursor(); }
        else { if (c.currentScreen == null) c.mouse.lockCursor(); ConfigManager.save(); }
    }
    ...
    private static void tick(MinecraftClient c) {
        if (menuKey != null && menuKey.wasPressed() && c.currentScreen == null) c.setScreen(new TopuHudScreen(null));
        if (editKey != null && editKey.wasPressed()) setEditMode(!editMode);
        if (sneakKey != null && sneakKey.wasPressed()) sneakToggled = !sneakToggled;
        if (c.player == null) return;
        TopuHudConfig cfg = ConfigManager.get();
        if (cfg.toggleSneak) c.player.setSneaking(sneakToggled);
        if (cfg.autoSprint && c.currentScreen == null && c.options.forwardKey.isPressed() && !c.player.isSneaking() && c.player.getHungerManager().getFoodLevel() > 6) c.player.setSprinting(true);
        long now = System.nanoTime(), delta = now - lastTickNanos;
        lastTickNanos = now;
        if (delta > 0) {
            double rate = Math.min(20.0, 1_000_000_000.0 / delta);
            tpsEstimate = MathHelper.clamp(tpsEstimate * 0.94 + rate * 0.06, 0.0, 20.0);
        }
    ...
    public static boolean isEnabledForEditor(HudId id) {
        ...
        switch (id) {
            case ARMOR:return c.armorHud; case FPS:return c.fpsCounter; case PING:return c.pingDisplay; case TPS:return c.tpsDisplay;
            case CPS:return c.cpsDisplay; case COMBO:return c.comboCounter; case TOTEM:return c.totemCounter; case POTION:return c.potionCounter;
            case EFFECTS:return c.potionEffects; case GAPPLE:return c.gappleCounter; case WARNING:return c.armorWarning; case ENEMY:return c.enemyHealth;
            case COOLDOWN:return c.cooldown; case BLOCK_OVERLAY:return c.blockOverlay; case KEYSTROKES:return c.keystrokes; case MEMORY:return c.memory;
            default:return false;
        }
    }

    public static int getWidthForEditor(HudId id) {
        switch (id) { case ARMOR:return 120; case EFFECTS:return 180; case WARNING:return 180; case ENEMY:return 165; case COOLDOWN:return 130; case KEYSTROKES:return 110; case BLOCK_OVERLAY:return 150; default:return 150; }
    }

    public static int getHeightForEditor(HudId id) {
        switch (id) { case ARMOR:return 36; case KEYSTROKES:return 70; case BLOCK_OVERLAY:return 80; case EFFECTS:return 40; default:return 18; }
    }

    private static void render(MinecraftClient c, DrawContext d, float delta) {
        if (c.player == null || c.world == null) return;
        TopuHudConfig cfg = ConfigManager.get();
        draw(c,d,cfg,HudId.FPS,cfg.fpsCounter,"FPS: "+c.getCurrentFps());
        draw(c,d,cfg,HudId.PING,cfg.pingDisplay,"Ping: "+ping(c)+" ms");
        draw(c,d,cfg,HudId.TPS,cfg.tpsDisplay,"TPS: "+String.format(java.util.Locale.ROOT,"%.1f",tpsEstimate));
        draw(c,d,cfg,HudId.CPS,cfg.cpsDisplay,"CPS: "+clickTimes.size());
        draw(c,d,cfg,HudId.COMBO,cfg.comboCounter,"Combo: "+combo);
        draw(c,d,cfg,HudId.COOLDOWN,cfg.cooldown,"Cooldown: "+Math.round(c.player.getAttackCooldownProgress(delta)*100)+"%");
        draw(c,d,cfg,HudId.MEMORY,cfg.memory,"Memory: "+usedMemory()+" MB");
        draw(c,d,cfg,HudId.ENEMY,cfg.enemyHealth,true,targetText(c));
        draw(c,d,cfg,HudId.ARMOR,cfg.armorHud,true,armorText(c.player));
        draw(c,d,cfg,HudId.KEYSTROKES,cfg.keystrokes,true,keys(c));
    }

    private static void draw(MinecraftClient c, DrawContext d, TopuHudConfig cfg, HudId id, boolean enabled, String text) { draw(c,d,cfg,id,enabled,false,text); }
    private static void draw(MinecraftClient c, DrawContext d, TopuHudConfig cfg, HudId id, boolean enabled, boolean force, String text) {
        if (!enabled || (!force && text == null)) return;
        int[] p=getPositionForEditor(cfg,id); d.fill(p[0]-3,p[1]-2,p[0]+getWidthForEditor(id),p[1]+16,0xA0101520); d.drawTextWithShadow(c.textRenderer,Text.literal(text),p[0],p[1],0xFFE7EDF7);
    }

    private static String armorText(PlayerEntity p) {
        ItemStack h=p.getEquippedStack(EquipmentSlot.HEAD), ch=p.getEquippedStack(EquipmentSlot.CHEST), l=p.getEquippedStack(EquipmentSlot.LEGS), f=p.getEquippedStack(EquipmentSlot.FEET);
        return "Armor: "+dur(h)+" | "+dur(ch)+" | "+dur(l)+" | "+dur(f);
    }
