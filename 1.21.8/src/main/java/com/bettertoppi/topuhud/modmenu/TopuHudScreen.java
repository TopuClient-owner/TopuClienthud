...
public final class TopuHudScreen extends Screen {
    ...
    @Override public void render(DrawContext d,int mx,int my,float delta){
        d.fill(0,0,width,height,0xFF080B11); d.fill(0,0,width,62,0xFF101722); d.fill(0,62,172,height,0xFF0C1119); d.fill(172,62,174,height,0xFF263244);
        d.fill(202,67,width-24,70,0xFF263244); d.fill(202,76,width-24,height-18,0x66141C29);
        d.drawText(textRenderer,Text.literal("TOPU"),24,20,0xFFFFFFFF,true); d.drawText(textRenderer,Text.literal("HUD"),76,20,0xFF6FA8FF,true);
        d.drawText(textRenderer,Text.literal("50 BUILT-IN UTILITIES"),24,40,0xFF7F8CA1,false); d.drawText(textRenderer,Text.literal("RIGHT SHIFT"),width-92,40,0xFF6F7D91,false);
        String active=category==null?"ALL UTILITIES":pretty(category.name()); d.drawText(textRenderer,Text.literal(active),202,48,0xFFE5ECF7,true);
        TopuUtilityManager.Utility[] filtered=TopuUtilityManager.search(searchBox==null?"":searchBox.getText(),category); d.drawText(textRenderer,Text.literal(filtered.length+" modules"),width-88,48,0xFF6F7D91,false);
        d.drawText(textRenderer,Text.literal("CATEGORIES"),22,64,0xFF5E6D82,true);
        d.drawText(textRenderer,Text.literal("TOPU UTILITY CONTROL"),202,72,0xFF65758A,false);
        if(filtered.length==0)d.drawText(textRenderer,Text.literal("No utilities found"),202,100,0xFF8B97AA,false);
