...
public final class TopuUtilityHud {
    ...
    private static void tick(MinecraftClient c){
        ...
        boolean autoSprint=on("autosprint");
        if(autoSprint&&c.currentScreen==null&&c.options.forwardKey.isPressed()&&!p.isSneaking()&&p.getHungerManager().getFoodLevel()>6){p.setSprinting(true);}
        boolean z=on("zoom");if(z&&!lastZoom){savedFov=c.options.getFov().getValue();c.options.getFov().setValue(Math.min(savedFov,35));}else if(!z&&lastZoom)c.options.getFov().setValue(savedFov);lastZoom=z;
        boolean f=on("fullbright");if(f&&!lastFullbright){savedGamma=c.options.getGamma().getValue();c.options.getGamma().setValue(16.0D);}else if(!f&&lastFullbright)c.options.getGamma().setValue(savedGamma);lastFullbright=f;
        boolean l=on("fpslimit");if(l&&!lastFpsLimit){savedMaxFps=c.options.getMaxFps().getValue();c.options.getMaxFps().setValue(Math.min(savedMaxFps,240));}else if(!l&&lastFpsLimit)c.options.getMaxFps().setValue(savedMaxFps);lastFpsLimit=l;FPS_HISTORY[fpsIndex++%FPS_HISTORY.length]=c.getCurrentFps();}
